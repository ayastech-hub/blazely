...
function TableHeader() {
  return (
    <div className="flex items-stretch border-b border-white/[0.08] min-w-max">
      <div
        className={`sticky left-0 z-20 ${STICKY_COL_WIDTH} shrink-0 flex items-center px-4 py-2.5 bg-[var(--bg-alt-2)]/95 backdrop-blur-md border-r border-white/[0.08] text-[10px] text-[var(--text-faint-2)] font-semibold`}
      >
        Token
      </div>
      <div className="flex items-center gap-3 px-4 py-2.5 text-[10px] text-[var(--text-faint-2)] font-semibold">
  ...
  return (
    <motion.div
      ...
      className={`border-b border-white/[0.05] last:border-b-0 transition-colors duration-300 min-w-max ${
        isNew ? "bg-[var(--teal)]/[0.05]" : "hover:bg-white/[0.035]"
      }`}
    >
      <Link to={`/token/${token.address}`} className="flex items-stretch group">
        ...
        <div
          className={`sticky left-0 z-10 ${STICKY_COL_WIDTH} shrink-0 flex items-center gap-3 px-4 py-3 bg-[var(--bg-alt-2)]/95 backdrop-blur-md border-r border-white/[0.08] ${
            isNew ? "bg-[var(--teal)]/[0.08]" : ""
          }`}
        >
          ...
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-[var(--text-faint-2)] font-semibold font-mono truncate">${token.symbol ?? "—"}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 px-4 py-3">
          ...
          <div className="w-24 shrink-0 text-right text-[var(--text-mid)] text-[11px] font-semibold font-mono tabular-nums">
  ...
  if (!data.length) {
    return (
      <div className={`text-center py-14 rounded-[22px] border-dashed ${NESTED_FILL}`}>
        <p className="text-sm text-[var(--text-mid-2)] font-medium">No tokens found</p>
        <p className="text-xs text-[var(--border-mid)] mt-1">Try a different search or clear your filters.</p>
      </div>
    );
  }
